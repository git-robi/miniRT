...
/*   Updated: 2025/03/12 09:01:14 by tatahere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_cylinder *cylinder)
{
	...
	t = vec3_dot_product(vec3_subtract(cylinder->position, ray_origin),
	...
	t = vec3_dot_product(vec3_subtract(top_center, ray_origin),
			cylinder->orientation) / vec3_dot_product(ray, cylinder->orientation);
	if (t > 0 && t < t_min)
	{
		p = vec3_add(ray_origin, vec3_scale(ray, t));
		...
		t_cylinder *cylinder)
{
	...
	radius = cylinder->diameter / 2.0;
	oc = vec3_subtract(ray_origin, cylinder->position);
	...
	p = vec3_add(ray_origin, vec3_scale(ray, t));
	h = vec3_dot_product(vec3_subtract(p, cylinder->position), v);
